...
void loopback_audio_buffer_callback(MiniAVLoopbackContextHandle context_handle,
  ...
  if (buffer->type == MINIAV_BUFFER_TYPE_AUDIO) {
    ...
    printf("LoopbackTestCallback: Audio buffer #%d received: %zu bytes, Timestamp: %" PRIu64 " us, "
           "Frames: %u, Format: %d, Channels: %u, Rate: %u\n",
           g_loopback_buffer_count,
           data_size, buffer->timestamp_us,
           buffer_format_info->num_frames, buffer_format_info->format, buffer_format_info->channels,
           buffer_format_info->sample_rate);

  } else {
    fprintf(stderr, "LoopbackTestCallback: Received non-audio buffer type: %d\n", buffer->type);
  }
  
  // If the buffer has an internal_handle, it implies the library expects the user to release it.
  // This pattern is seen in the screen capture test. Adapt if your loopback API has a different buffer lifecycle.
  // If loopback audio buffers are e.g. pointers to an internal ring buffer and not individually allocated for the user,
  // then this release call would not be needed or appropriate.
...
void print_usage() {
  ...
  printf("  target_index (optional): Index of the loopback target to use (from enumerated list).\n");
  printf("                           If not provided, an interactive prompt or default selection will occur.\n");
  printf("  duration_seconds (optional): How long to capture in seconds. Default %d.\n", DEFAULT_CAPTURE_DURATION_SECONDS);
...
int main(int argc, char *argv[]) {
  ...
  int user_cli_target_idx = -2; // -2 indicates no CLI input, -1 would mean default from CLI
  ...
  res = MiniAV_Loopback_EnumerateTargets(MINIAV_LOOPBACK_TARGET_PROCESS, &targets, &target_count);
  ...
  if (target_count == 0) {
    ...
    if (user_cli_target_idx > -2) { // User provided a target index via CLI
        if (user_cli_target_idx >= 0 && (uint32_t)user_cli_target_idx < target_count) {
            ...
            fprintf(stderr, "\nInvalid target index %d from command line. Will attempt system default.\n", user_cli_target_idx);
        ...
        if (target_count == 1) {
            ...
            printf("\nEnter the index of the loopback target to capture (0-%u, or 'd' for system default): ", target_count - 1);
            if (fgets(user_input_buffer, sizeof(user_input_buffer), stdin)) {
                if (user_input_buffer[0] == 'd' || user_input_buffer[0] == 'D') {
                    ...
                    selected_target_id = NULL;
                } else if (sscanf(user_input_buffer, "%d", &final_selected_idx) == 1 &&
                           final_selected_idx >= 0 && (uint32_t)final_selected_idx < target_count) {
                 ...
                 fprintf(stderr, "\nFailed to read selection. Will attempt system default target.\n");
                 selected_target_id = NULL;
            }
        }
    }
    if (final_selected_idx != -1) { // A valid index was determined either by CLI, auto-selection, or interactive
        selected_target_id = targets[final_selected_idx].device_id;
    }
    // If selected_target_id is still NULL here, it means system default is intended.
  }
   if (!selected_target_id && target_count > 0 && user_cli_target_idx <= -2) { // Check if default was chosen explicitly or by lack of choice
        printf("\nAttempting system default loopback target.\n");
   } else if (!selected_target_id && target_count == 0) {
        printf("\nNo specific target available. Attempting system default loopback target.\n");
   }
  ...
  if (res != MINIAV_SUCCESS) {
    ...
    if (targets) MiniAV_FreeDeviceList(targets, target_count);
    return 1;
  }
  ...
  desired_format.format = MINIAV_AUDIO_FORMAT_S16;
  desired_format.channels = 2;
  desired_format.sample_rate = 48000;

  printf("\nConfiguring loopback capture for target_id: %s (NULL means system default)\n",
  ...
  res = MiniAV_Loopback_Configure(loopback_ctx, selected_target_id, &desired_format);
  if (res != MINIAV_SUCCESS) {
  ...
  printf("\nStarting loopback capture for %d seconds (or press Enter if duration is 0)...\n", capture_duration_seconds);
  g_loopback_buffer_count = 0;
  g_stop_requested = 0;
  res = MiniAV_Loopback_StartCapture(loopback_ctx, loopback_audio_buffer_callback, NULL);
  if (res != MINIAV_SUCCESS) {
  ...
  if (capture_duration_seconds > 0) {
    for (int i = 0; i < capture_duration_seconds; ++i) {
      printf("LoopbackTest main: Capturing... (%d/%d s), Buffers received so far: %d\n",
